...
class XYStatePlotter:
    ...
    def plot_stuff_so_far(self,
        ...
        if ax:
            # plt.scatter(all_states_x, all_states_y, c=all_colors, s=100)
    ...
    def make_q_value_plot_from_log(self, load_path, save=False, point_maze=True):
        with open(load_path, "rb") as f:
            data = pickle.load(f)
        self.env.maze.plot()
        x, y, v = data
        plt.scatter(x-0.5, y-0.5, c=v)
        plt.colorbar()
        plt.show()
