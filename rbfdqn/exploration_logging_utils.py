...
def _make_core_set(starting_set, starting_core_set=None, radius=1.):
    ...
    # This is sort of a weird way to do it, because all the doing code is in the next method...
    # Okay, now I understand. It's doing it quickly against the current core set, and then whatever
    ...
    while len(starting_set) != 0:
        ...
        # in_ball_indices = [i for i,d in enumerate(distances) if d < radius]
        out_of_ball_indices = [
            i for i, d in enumerate(distances) if d >= radius
        ]
        new_starting_set = np.take(others, indices=out_of_ball_indices, axis=0)
        # print(f"Old length: {len(others)}. Took out: {len(in_ball_indices)}. New length: {len(new_starting_set)}")
        # print("Old length: ", len(others), ". Took out: ", len(in_ball_indices), ". New length: ", len(new_starting_set))
...
def _filter_to_novel_states_with_core_set(*,
    ...
    """
    # I guess I can get all the distances, and then take the minimum distance.
...
def get_coreset_volume(states,
    ...
    How about, we want to figure out how many unique points are visited,
    versus the amount of total points? So, we make a core-set, and
    then see how many points it narrows. If it's doing a good job,
    this will stay high. If it does badly, this will become small,
    because everything is mapping to old points.

    Eventually, normalizer will be something that makes all the
    dimensions similar in their scale, so an eps-ball means
    something.
    """
...
class CoreSetCreator:
    ...
    I guess I can just keep two parallel lists. That's really not that bad.
    Only works with k=1 for now. Annoying.
    """
    ...
    def add_states(self, states):
        ...
        # It's not super efficient but that's okay.
        # We can actually do the filtering first, that's super fine.

        # First, get the normalized states and filter from coreset. like _filter_to_novel_states_with_core_set
        # states = list(states)
