...
def rbf_function_on_action(centroid_locations, action, beta):
    ...
    diff_norm = torch.sqrt(diff_norm +
                           1e-5)  # to match with what we had before...
...
class Net(nn.Module):
    def __init__(self,
        ...
        # assert not (use_exploration and use_knownness)
        self.use_exploration = use_exploration
        # assert not (use_rnd and use_knownness)
        ...
        if self.use_knownness:
            ...
            if self.use_torch_knownness:
                if self.only_state_knownness:
                    ...
                    # knownness_scaling_array=knownness_scaling_array)  # leave batch_size at 100 for now.
                else:
        ...
        if self.params.get('q_max') is not None:
            ...
            # self.q_max = self.params.get('q_max', )
    ...
    def get_best_qvalue_and_action(self,
        ...
        if self.use_shaping:
            # print("What type is s? {}".format(type(s)))
        ...
        if novelty_tracker is not None and use_exploration_if_enabled:
    ...
    def enact_policy(self, s, episode, train_or_test, policy_type="e_greedy"):
        # TODO: Doesn't pass through use_exploration_if_enabled. I think that's fine.
    ...
    def update(self, target_Q, return_logging_info=False):
        ...
        with torch.no_grad():
            if self.use_knownness:
                ...
                # print("using RND for update")
                Q_star, _ = target_Q.get_best_qvalue_and_action(
                    sp_matrix,
                    novelty_tracker=None,
                    use_exploration_if_enabled=False)
                # print(f"Q_star.shape is {Q_star.shape}")
                rnd_bonus = self.rnd_module.get_exploration_bonus_for_states(sp_matrix)
                # print(f"RMS rnd bonus: {torch.sqrt(torch.square(rnd_bonus).mean()).item()}")
                # print(f"rnd_bonus.shape is {rnd_bonus.shape}")
    ...
    def add_trajectory_to_novelty_tracker(self, trajectory):
        states = np.array([s for s, a, r, ns, d in trajectory])
        actions = np.array([a for s, a, r, ns, d in trajectory])
        self.novelty_tracker.add_many_transitions(states, actions)
        if self.use_exploration:
    ...
    def add_trajectory_to_replay_buffer(self, trajectory):
        ...
        if self.use_exploration or self.use_knownness:
            self.add_trajectory_to_novelty_tracker(trajectory)
