...
def torch_get_all_distances_to_buffer(states, buffer):
    """
    This uses a trick, that (a - b)^2 = a^2 + b^2 - 2ab. And we can do those with much
    less space!

    But we can also just use the cdist function... Look at that. It's around 3 times faster. 

    Interestingly, it doesn't seem like cdist helps overall speed, even though I assumed this was the bottleneck, and it makes this like 3-5 times faster.
    Wonder what the deal is. I thought that this would be the slow part...
...
class MPEExploration:
    ...
    def update(self, states, actions, next_states):
        ...
        We want this all to be the same datatype as whatever is being passed in.
        On GPU etc. That's fine.

        It seems like we pass numpy stuff to the other novelty guys. But for this one,
        let's not, because it has to train.
        """
...
class BaseNoveltyMixin:
    ...
    def perform_normalization(self):
        ...
        # print("Hey, knownness scaling array is {}".format(knownness_scaling_array))
        ...
        if getattr(self, 'action_scaling', None):
            print("Action scaling is a thing of the past!")
    ...
    def _num_epsilons_to_knownness(self, num_epsilons_away):
        ...
        elif mapping_type == "old_polynomial":
            # This is what polynomial looked like before. It looks like exponential at the origin, but falls off much slower. 
...
class ApproxNoveltyMixin(BaseNoveltyMixin):
    ...
    def _add_many_transitions_without_normalization(self, states, actions=None):
        """
        This will manage the filtering and all that nonsense. It may need to make arrays for the new states

        Seems as though the normalization of the states actually happens inside of _post_add_transitions. That's fine.
        We just need some way that we can do it during the calculation itself. But then we need to add the un-normalized states.
        """
        # Set it up!
...
class TorchKnownnessMixin:
    ...
    def get_knownness(self, states, actions=None):
        ...
        # self.tree.query(states, k=self.m, distance_upper_bound=self.epsilon*10, p=1.5)
        ...
        # It helps because the thing we're querying against is like 7680 long. Which matches perfectly --
        # 7680 * 80000 * 8 = 4.5 billion = 4.5 GB.
        topk_candidates = []
        for chunk in buffer_chunks:
            ...
            # m_or_batch_size = min(self.m, int(chunk.shape[1])) # THIS WAS WRONG!!! Should have been chunk.shape[0].
...
class StandardKnownnessMixin:
    ...
    def _post_add_transitions(self):
        ...
        # self.tree = cKDTree(normalized_buffer)
...
class OnlyStateExplorationClass:
    ...
    def get_batched_exploration_bonus(self, states, actions=None):
        ...
        # print("Mean exploration bonus: ", exploration_bonus.mean())
