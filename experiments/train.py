...
# I wish this wasn't necessary, but when we take things out of the argparse, we need a
# clean way to define their defaults.
PARAM_DEFAULTS = {
    "skip_normalization": False,  # You want to normalize by default.
...
def mcar_reward_shaping(s, sp, gamma=0.99):
    to_return = gamma*sp[0] - s[0]
    return to_return
...
if __name__ == "__main__":
    ...
    parser.add_argument("--visualize_values",
                        action="store_true",
                        default=False,
                        help="Logs for future value visualization")
    ...
    parser.add_argument(
        ...
        type=int,
        help=
        "Lets you set arbitrary number of no-ops before episode starts."
    )
    ...
    parser.add_argument("--mcar_reward_shaping_scale",
                        default=0,
                        type=float,
                        help="Defines how much we use reward shaping")
    ...
    assert not (args.random_agent and
                (args.use_knownness or args.use_rnd or args.use_mpe_bonus)), "can't do random with exploration."
    ...
    for episode in range(params['max_episode']):
        ...
        while done == False:
            ...
            if params["env_name"] == "MountainCarContinuous-v0" and args.mcar_reward_shaping_scale != 0:
                r += args.mcar_reward_shaping_scale*mcar_reward_shaping(s, sp, gamma=params["gamma"])
